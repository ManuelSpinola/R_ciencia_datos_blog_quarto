...
```{r}
cr <- st_read(here("vectors/cr_5367.gpkg"))
```
...
```{r}
ca <- rast(here("rasters/ca.tif"))
pa <- rast(here("rasters/pa.tif"))
cu <- rast(here("rasters/cu.tif"))
ze <- rast(here("rasters/ze.tif"))
hh <- rast(here("rasters/hh.tif"))
mg <- rast(here("rasters/mg.tif"))
```
...
```{r}
area_table <- import(here("data/area_table.xlsx"))
area_table
```
...
```{r}
slic <- st_read(here("vectors/supercells.gpkg"))
```
